...
| **C — value and returns** | `property_valuations` (`lib/property-valuations.ts`, manual / laudo / FipeZap-carried / anúncios) with a dialog in the analysis section; engine extended with market value, appreciation, equity (value − outstanding balance), equity multiple, total return, TIR com valorização, cap rate, and the real (IPCA, today's money) payback with a chart toggle; `GET /api/portfolio/metrics` runs the engine for every property; `/imoveis` gets a totals strip (investido, renda líquida, payback, yield líquido 12 m, valor de mercado) and each card shows payback %, forecast and yields | ✅ delivered (PR #46) |
| **D — scenarios, report, public calculator, bank statements** | `lib/investment-scenarios.ts` (`projectScenario`: rent growth, vacancy, extra amortisation via the mortgage schedule, appreciation, sale at year N with IRR) behind a "Cenários: e se…" panel in the analysis; `GET /api/properties/[id]/report` Excel workbook (Resumo, Série mensal, Receitas, Investimento, Tributos, Avaliações); public `/calculadora-payback-imovel` on `lib/payback-simulator.ts` (listed in the calculators overview, sidebar and sitemap); `lib/bank-statement.ts` OFX/CSV parser + memo classifier with a review dialog ("Importar extrato") writing `source = BANK` rows keyed by `bank_reference` | ✅ delivered (PR #47) |
| **D′ — Banco Inter API sync (Contábil & Fiscal)** | Lives in the new sidebar section **Contábil & Fiscal › Contas bancárias** (`/contabil/contas-bancarias`), not per property: the holding's account feeds the accounting side (DRE, balanço, impostos) and, through the same classifier, both property ledgers: tenant inflows matched to a property become BANK-sourced months in Receitas de Aluguel, classified outflows become investment transactions. Needs the Inter API credentials (mTLS certificate + key, client id/secret, account) stored as server secrets, a `bank_connections` table, a daily cron and a chart of accounts; PDF report can follow the Excel one | ⏳ API sync blocked on credentials. **Delivered meanwhile (PR #48):** holding-level statement import on the page — OFX, CSV/TXT (Inter layout with preamble and Histórico + Descrição) and PDF (Gemini) — with routing suggestions (previous imports → property name/tenant in the memo → kind from the memo), a review table, the `bank_transactions` ledger (one row per bank reference, `destination` INVESTMENT / INCOME / IGNORED, `linked_id`) and dispatch to `property_transactions` (source BANK) or `property_income_months` (received amount of the month, source BANK) |

**Money model (as of 2026-09-15).** Per calendar month: invested = everything paid — entrada + custos de aquisição + prestações + amortizações + quitação + tarifas bancárias + reformas + utilidades + outros + energia solar + every landlord-paid tax (IPTU, ITBI, outros) from the taxes register in the month it was paid (the register is the only tax source: the ledger's IPTU column, the investment ledger's IPTU kind and the old "Ajustar Custos" setting are gone); net rent = received − energy portion; property opex = outras despesas (paid out of the rent); NOI = net rent − property opex + net energy (energy − custo de energia). The solar system is no longer a separate payback centre in the engine (its tile still shows its own recovery). "Total investido no imóvel" on the page = Entrada + Banco (with tarifas) + Reformas + Custos do imóvel (with taxes) + Energia solar. Payback % = Σ NOI ÷ Σ invested (cash basis). The forecast walks forward at the average NOI of the months with income in the last 12, adding the remaining instalments while the loan is active. TIR realizada = XIRR of the monthly (NOI − invested) flows dated mid-month, no sale. On the property page everything is computed client-side from data the page already loads; `/api/portfolio/metrics` (slice C) runs the same engine server-side for the whole portfolio. FipeZap valuations carry the purchase price by the national sale index of the property's bedroom bucket from the acquisition month; the owner confirms and saves the estimate, it is never written silently. Real payback restates every month's invested and NOI to today's money by the IPCA level (`priceLevelFactors`).

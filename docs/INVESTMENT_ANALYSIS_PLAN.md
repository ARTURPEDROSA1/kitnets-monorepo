...
**Slice B money model (as implemented).** Per calendar month: invested = entrada + custos de aquisição + prestações + amortizações + quitação + reformas; running costs = utilidades + outros, plus every landlord-paid tax (IPTU, ITBI, outros) from the taxes register in the month it was paid; tarifas bancárias count as investment (the account exists for the loan) (since 2026-09-15 the register is the only IPTU source: the ledger's IPTU column and the investment ledger's IPTU kind are ignored, and the "Ajustar Custos" setting is gone); net rent = received − energy portion; property opex = outras despesas; net energy (energy − custo de energia) pays the solar system back first and only the surplus counts for the property; NOI = net rent − property opex − running costs + energy surplus. Payback % = Σ NOI ÷ Σ invested (cash basis). The forecast walks forward at the average NOI of the months with income in the last 12, adding the remaining instalments while the loan is active. TIR realizada = XIRR of the monthly (NOI − invested) flows dated mid-month, no sale. On the property page everything is computed client-side from data the page already loads; `/api/portfolio/metrics` (slice C) runs the same engine server-side for the whole portfolio. FipeZap valuations carry the purchase price by the national sale index of the property's bedroom bucket from the acquisition month; the owner confirms and saves the estimate, it is never written silently. Real payback restates every month's invested and NOI to today's money by the IPCA level (`priceLevelFactors`).

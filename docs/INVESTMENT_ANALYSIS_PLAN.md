...
| **D′ — Banco Inter API sync** | Server-side sync of the Inter account statement into the ledgers using the same classifier: needs the Inter API credentials (mTLS certificate + key, client id/secret, account) stored as secrets, a `bank_connections` table and a cron; PDF report can follow the Excel one | ⏳ blocked on credentials |

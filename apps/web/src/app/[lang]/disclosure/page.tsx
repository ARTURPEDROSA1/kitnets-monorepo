...
export default async function DisclosurePage({ params }: { params: Promise<{ lang: "en" | "pt" | "es" }> }) {
    const { lang } = await params;
    const content = disclosureContent[lang as keyof typeof disclosureContent] || disclosureContent.pt;
                ...
                <div className="space-y-8">
                    {content.sections.map((section, index) => (
                        <section key={index}>
                            ...
                            {section.footer && (
                                <p className="text-sm italic text-muted-foreground mt-2 whitespace-pre-line">
                                    {section.footer}
                                </p>
                            )}

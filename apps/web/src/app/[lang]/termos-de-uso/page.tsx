...
export default async function TermsPage({ params }: { params: Promise<{ lang: "en" | "pt" | "es" }> }) {
    const { lang } = await params;
    const content = termsContent[lang as keyof typeof termsContent] || termsContent.pt;
                        ...
                        <section key={index}>
                            ...
                            {section.subsections && section.subsections.map((sub, subIndex) => (
                                <div key={subIndex} className="mt-4 mb-4">
                                    ...
                                    <p className="mb-2 text-foreground/80 whitespace-pre-line">{sub.content}</p>
                                    {sub.items && (
                                        <ul className="list-disc pl-6 mb-4 space-y-1 text-foreground/80">

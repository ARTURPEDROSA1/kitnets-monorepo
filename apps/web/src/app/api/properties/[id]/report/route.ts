...
export async function GET(_request: Request, context: RouteContext) {
    ...
    try {
        ...
        sr.columns = [
            ...
            { header: "Taxa sobre o condomínio", key: "feeOnCondo", width: 14 }, { header: "Taxa da imobiliária (R$)", key: "fee", width: 16 },
            { header: "NOI", key: "noi", width: 13 }, { header: "Status", key: "status", width: 12 }, { header: "Comentários", key: "notes", width: 40 },
        ];
        for (const r of incomeRows) {
            const b = breakdown(r);
            sr.addRow({ month: formatMonthKey(monthKey(r.month)), unit: r.unit_name ?? "", gross: b.grossRent, pct: b.feePct, net: b.netRent, energy: b.energy, received: b.received, other: b.other, otherExp: b.otherExpenses, condo: b.condo, feeOnCondo: b.condo > 0 ? (b.feeOnCondo ? "Sim" : "Não") : "", fee: b.feeAmount, noi: b.noi, status: r.status === "CONFIRMED" ? "Confirmado" : "Previsto", notes: r.notes ?? "" });
        }
        styleTable(sr, ["gross", "net", "energy", "received", "other", "otherExp", "condo", "fee", "noi"]);

...
export async function GET() {
    ...
    const [props, invs, txs, incomes, taxes, vals, ipca] = await Promise.all([
        ...
        supabase.from("property_income_months").select("id, property_id, month, unit_id, unit_name, received_on, received_amount, energy_portion, other_income, other_expenses, condo_amount, fee_on_condo, iptu_amount, agency_fee_pct, status, source, bank_reference, notes").eq("owner_id", profileId),

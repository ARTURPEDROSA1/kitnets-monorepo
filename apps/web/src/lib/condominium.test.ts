...
describe("condominium months", () => {
    it("adds the units' condominium as the month's revenue and subtracts the month's costs", () => {
        const months = buildCondominiumMonths(
            ...
            [cost("2026-09", { internet_cost: 120.5, maintenance_cost: 80, notes: "lâmpadas" })]
        );
    ...
    it("energy, water and IPTU come from the bills and the taxes register, never from the cost row", () => {
        const months = buildCondominiumMonths(
            [income("2026-09", "u1", 300), income("2026-08", "u1", 300)],
            [cost("2026-09", { energy_cost: 999, iptu_amount: 999, water_cost: 999, internet_cost: 40 })],   // stale typed values are ignored
            { energy: new Map([["2026-09", 180.4], ["2026-07", 150]]), water: new Map([["2026-09", 329.16]]), iptu: new Map([["2026-08", 90]]) }
        );
    ...
    it("summarises revenue, costs by kind, result and margin", () => {
        const months = buildCondominiumMonths(
            [income("2026-09", "u1", 300), income("2026-08", "u1", 300)],
            [cost("2026-09", { maintenance_cost: 100, internet_cost: 50 }), cost("2026-08", { internet_cost: 30 })]
        );
        const s = summarizeCondominium(months);
        expect(s).toMatchObject({ months: 2, revenue: 600, totalCost: 180, result: 420, marginPct: 70 });
        expect(s.byCost).toEqual({ energy_cost: 0, internet_cost: 80, water_cost: 0, iptu_amount: 0, maintenance_cost: 100 });
    ...
    it("card figures: the latest month and the current year to date", () => {
        const months = buildCondominiumMonths(
            ...
            [cost("2026-09", { internet_cost: 100 }), cost("2025-12", { internet_cost: 50 })]
        );
